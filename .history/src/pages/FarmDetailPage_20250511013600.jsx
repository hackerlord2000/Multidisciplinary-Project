...
export default function FarmDetailPage() {
  ...
  const status = mockStatus[farmId?.toUpperCase()] || { temperature: 0, humidity: 0, sunlight: 0 };

  // Giá trị điều chỉnh (có thể khác trạng thái hiện tại)
  const [temperature, setTemperature] = useState(status.temperature);
  const [humidity, setHumidity] = useState(status.humidity);
  const [sunlight, setSunlight] = useState(status.sunlight);
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-around", marginBottom: 32 }}>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ color: "#2e7d32", fontWeight: 700, fontSize: 20 }}>{status.temperature}°C</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 28 }}>💧</div>
            <div style={{ color: "#333", fontWeight: 600 }}>Humidity</div>
            <div style={{ color: "#2e7d32", fontWeight: 700, fontSize: 20 }}>{status.humidity}%</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 28 }}>☀️</div>
            <div style={{ color: "#333", fontWeight: 600 }}>Sunlight</div>
            <div style={{ color: "#2e7d32", fontWeight: 700, fontSize: 20 }}>{status.sunlight}%</div>
          </div>
        </div>
        {/* Điều chỉnh */}
